...
> Read more: [`charmcraft` | File `requirements.txt`]()
...
> Read more: [`charmcraft` | File `src/charm.py`]()

This file needs to be executable. In your charm's root directory run:

```bash
chmod a+x src/charm.py
```

Now, in this file, do all of the following:
...
```
> Read more: [`ops.CharmBase`](https://ops.readthedocs.io/en/latest/index.html#ops.CharmBase)
...
> See more: {ref}`run-workloads-with-a-charm-kubernetes`
